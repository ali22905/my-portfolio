    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        <Link to="/" className="flex items-center gap-2" onClick={
          ...
          Ali Attia <span className="sm:block hidden">| web and AI developer</span>
          </p>
        </Link>
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          <ul className='list-none flex sm:justify-end flex-col gap-4'>
            {navLinks.map((link) => (
              <li
              key={link.id}
              onClick={() => {
              ...
              style={{transition: '.3s'}} 
              className={`${active === link.title ? "text-white" : "text-secondary"} font-poppines font-medium cursor-pointer text-[16px]`}
              >
                <a href={`#${link.id}`}>{link.title}</a>
              </li>
            ))}
